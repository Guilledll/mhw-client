...
<template>
  <nav class="relative items-center p-2">
    <div class="border border-transparent hover:border-gray-50 hover:shadow-md transition-shadow rounded-lg flex justify-between px-4 py-2">
      <div>
      ...
      <UDropdown :items="items" :ui="{ item: { disabled: 'cursor-default select-text opacity-75' } }" :popper="{ placement: 'bottom-end' }">
        <UAvatar
        ...
        <template #item="{ item }">
          <span class="truncate font-normal text-gray-700 dark:text-white"> {{ item.label }}</span>
          <UIcon :name="item.icon" class="flex-shrink-0 h-4 w-4 text-gray-600 dark:text-gray-500 ms-auto" />
        </template>
      </UDropdown>
    </div>
  </nav>
